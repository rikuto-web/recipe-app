# 画面遷移

## 0. UI プロトタイプ

**Pattern A（ウォーム・カード型）** を採用する。ビジュアル・レイアウトの詳細は [UI デザイン](08-ui-design.md) を参照。

| 画面 | Pattern A での表現 |
| --- | --- |
| SC-01 一覧 | カードグリッド（テーブルではない）。フィルタは上部の白カード内 |
| SC-02 詳細 | 1 カラム。説明は左ボーダー付きブロック、手順は番号付き円アイコン |
| SC-03 / SC-04 フォーム | セクション分割（基本情報 / 材料 / 手順）。pill 型ボタン |

参照: [`prototypes/pattern-a/index.html`](../prototypes/pattern-a/index.html)

## 1. 画面一覧

| ID | 画面名 | パス | 主な操作 |
| --- | --- | --- | --- |
| SC-01 | レシピ一覧 | `/recipes` | 一覧表示、検索・フィルタ、新規作成、詳細へ、削除 |
| SC-02 | レシピ詳細 | `/recipes/$id` | 詳細表示、人数按分、編集へ、削除、一覧へ |
| SC-03 | レシピ新規作成 | `/recipes/new` | 入力、材料・手順の行追加/削除、保存、キャンセル時は一覧へ |
| SC-04 | レシピ編集 | `/recipes/$id/edit` | 親の編集、材料・手順の行単位保存、キャンセル時は詳細へ |

削除専用画面は作らない。確認ダイアログで完結する。

## 2. 画面遷移図

```mermaid
stateDiagram-v2
  [*] --> List: 初回アクセス
  List --> Detail: レシピ選択
  List --> New: 新規作成
  Detail --> List: 戻る
  Detail --> Edit: 編集
  New --> Detail: 保存成功
  New --> List: キャンセル
  Edit --> Detail: 保存成功
  Edit --> Detail: キャンセル
  Detail --> List: 削除成功
  List --> List: 削除成功
```

## 3. ルーティングと search params

TanStack Router でパスとクエリを型安全に扱う。

| 画面 | パス | search params（例） |
| --- | --- | --- |
| 一覧 | `/recipes` | `q`, `category_id`, `difficulty`, `max_cook_time`, `sort` |
| 詳細 | `/recipes/$id` | なし（人数按分は画面内 state） |
| 新規 | `/recipes/new` | なし |
| 編集 | `/recipes/$id/edit` | なし |

一覧のフィルタ条件は URL に載せ、ブックマークや再読み込みで同じ条件を復元できるようにする。

## 4. 画面別ワイヤーフレーム（要素）

### SC-01 レシピ一覧

```
+--------------------------------------------------+
| レシピ管理                              [新規作成] |
+--------------------------------------------------+
| キーワード [________]  カテゴリ [v]  難易度 [v]    |
| 調理時間上限 [__]分   並び [新しい順 v]  [検索]   |
+--------------------------------------------------+
| タイトル        | カテゴリ | 時間 | 難易度   | 操作 |
| 醤油ラーメン    | 和食     | 30分 | ★★★☆☆ | ...  |
| チーズケーキ    | デザート | 60分 | ★★★★☆ | ...  |
+--------------------------------------------------+
| 該当 2 件                                        |
+--------------------------------------------------+
```

- 操作: 詳細へ、削除（確認ダイアログ）
- 0 件時: 「レシピがありません」「条件を変えて検索してください」等

### SC-02 レシピ詳細

```
+--------------------------------------------------+
| [< 一覧]                    [編集] [削除]        |
+--------------------------------------------------+
| 醤油ラーメン                                     |
| 和食 / 30分 / ★★★☆☆ / 2人分                       |
| 説明文...                                        |
+--------------------------------------------------+
| 表示人数 [ 4 ] 人  （基準: 2人分）               |
| 材料                                             |
|   中華麺    240 g                                |
|   豚バラ    160 g                                |
|   ...                                            |
| 手順                                             |
|   1. スープを作る                                |
|   2. 麺を茹でる                                  |
+--------------------------------------------------+
```

### SC-03 / SC-04 新規作成・編集

共通フォーム。編集は既存値を loader で取得して初期表示する。

**キャンセル時の遷移**

| 画面 | 遷移先 | 理由 |
| --- | --- | --- |
| 新規作成（SC-03） | 一覧 `/recipes` | まだレシピが存在しないため |
| 編集（SC-04） | 詳細 `/recipes/$id` | 編集前に見ていたレシピへ戻る |

未保存の変更がある場合は確認ダイアログを出してから遷移する。

```
+--------------------------------------------------+
| レシピを作成 / 編集                              |
+--------------------------------------------------+
| タイトル * [________________]                    |
| カテゴリ * [和食 v]                              |
| 人数 * [2]  調理時間 * [30] 分（10分単位）  難易度 * [★★★☆☆]     |
| 説明   [________________________]                |
+--------------------------------------------------+
| 材料 *                          [+ 行を追加]     |
|   材料名 *   分量 *   単位 *                     |
|   [_______] [___] [g v]                 [削除]   |
|   [_______] [___] [___]                 [削除]   |
+--------------------------------------------------+
| 手順 *                          [+ 行を追加]     |
|  ① [________________________]           [削除]    |
|  ② [________________________]           [削除]    |
+--------------------------------------------------+
| [キャンセル]                         [保存]      |
+--------------------------------------------------+
```

（SC-04 編集時は材料・手順行に行単位の保存ボタンあり。SC-03 は一括保存のみ。）

```
+--------------------------------------------------+
| レシピを編集                                     |
+--------------------------------------------------+
| （基本情報は SC-03 と同様）                      |
+--------------------------------------------------+
| 材料 *                          [+ 行を追加]     |
|   材料名 *   分量 *   単位 *                     |
|   [_______] [___] [g v]        [保存] [削除]     |
+--------------------------------------------------+
| 手順 *                          [+ 行を追加]     |
|  ① [________________]          [保存] [削除]      |
+--------------------------------------------------+
| [キャンセル]                  [親情報を保存]     |
+--------------------------------------------------+
```

- 材料・手順行は **行ごとに保存** できる（`PATCH`）。親情報の保存ボタンとは独立。
- **新規作成（SC-03）** は材料・手順も含めて一括保存。キャンセルは一覧へ。
- **編集（SC-04）** は材料・手順をピンポイント更新可能。親情報は `PUT`、子行は `PATCH`。キャンセルは詳細へ。

## 5. エラー・空状態

| 状況 | 画面 | 振る舞い |
| --- | --- | --- |
| 一覧 0 件 | 一覧 | 空状態 UI |
| 存在しない ID | 詳細 / 編集 | 404 メッセージ → 一覧へ |
| API 失敗 | 全般 | トーストまたはインラインエラー |
| バリデーションエラー | 新規 / 編集 | フィールド横にエラー表示。触れたフィールドは空/不正の時点で表示。保存時は全体チェック |

### SC-03 新規作成フォーム（VS-03 実装）

- **調理時間**: `CookTimeStepper`（±10 分）。最小 10 分。
- **難易度**: `DifficultySelector`（星クリック）。
- **材料**: 列見出し（材料名 / 分量 / 単位）をセクション上部に 1 回だけ表示。各行は入力欄 + 削除のみ。
- **単位**: `UnitInput`（候補ドロップダウン + 自由入力。`<datalist>` は使わない）。
- **手順**: 行頭に番号付き円アイコンのみ（「手順 1」ラベルは出さない）。テキストエリア + 削除。
- **バリデーション**: 触れたフィールドは blur/入力時にそのフィールドだけエラー表示。未触れの必須項目は保存時まで指摘しない。全角数字は半角に正規化。
- **保存成功**: 詳細 `/recipes/$id` へ遷移。二重送信防止あり。

## 6. ユースケースとの対応

| ユースケース | 画面 |
| --- | --- |
| UC-01, UC-02 | SC-01 |
| UC-03, UC-04, UC-07 | SC-02（UC-07 は SC-01 からも可） |
| UC-05 | SC-03 |
| UC-06 | SC-04 |
| UC-08 | SC-03, SC-04, SC-01（フィルタ） |
