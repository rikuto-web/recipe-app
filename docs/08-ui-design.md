...
- セクション分割（基本情報 / 材料 / 手順）
- **調理時間**: `CookTimeStepper`（10 分単位、新規作成は最小 10 分）
- **難易度**: `DifficultySelector`（`Star` クリック、`DifficultyRating` とは別コンポーネント）
- **材料行**: 列見出し（材料名 / 分量 / 単位）をセクション上部に 1 回表示。各行は入力 + 削除（編集時は行保存も）
- **単位**: `UnitInput`（入力欄直下の候補リスト + 自由入力。`<datalist>` は位置ずれするため非採用）
- **手順行**: 番号付き円アイコン + テキストエリア + 削除（「手順 N」見出しは重複するため非表示、`aria-label` で識別）
- **バリデーション**: 触れたフィールドのみ即時エラー。保存時に全体チェック。エラーは `aria-describedby` で関連付け
